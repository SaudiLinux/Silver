...
Description: Comprehensive security testing tool for authorized penetration testing
"""
...
╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    def run_sql_injection_tests(self, target_url: str, parameters: List[str] = None) -> Dict[str, Any]:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python dr-sayer.py -u http://example.com --sql
  python dr-sayer.py -u http://example.com --xss --log4j
  python dr-sayer.py -u http://example.com --all --report html
        """
    )
    ...
    parser.add_argument('--oob-callback', help='Callback host for OOB probes (e.g., collaborator.example.com)')
    ...
    if args.sql_oob and args.oob_callback:
        tool.sql_tester.enable_oob = True
        tool.sql_tester.oob_callback = args.oob_callback
    
    try:
        # Run selected tests
        if args.all or args.sql:
            tool.run_sql_injection_tests(args.url, args.params)
